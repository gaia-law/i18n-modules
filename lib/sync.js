...
const build = ({ keysRoot, moduleEnding, dictionaryPattern, dictionaryAdapter }) => {
  ...
  Object.keys(generated).forEach((language) => {
    ...
    const seed = fs.existsSync(filePath) ? dictionaryAdapter.parse(fs.readFileSync(filePath)) : {};

    const dictionary = getSortedObject({ ...getCleanSeed(seed), ...generated[language] });
    const content = dictionaryAdapter.stringify(dictionary);
...
const update = ({ keysRoot, moduleEnding, dictionaryPattern, dictionaryAdapter }) => {
  const startTime = Date.now();
  ...
  const moduleValues = getFiles(dictionariesRoot, dictionaryRegex).reduce((acc, filePath) => {
    ...
    const dictionary = dictionaryAdapter.parse(fs.readFileSync(filePath));
...
const clean = ({ dictionaryPattern, dictionaryAdapter }) => {
  const startTime = Date.now();
  ...
  getFiles(dictionariesRoot, dictionaryRegex).forEach((filePath) => {
    ...
    const content = dictionaryAdapter.stringify(seed);
