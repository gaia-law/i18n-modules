...
describe('utils', () => {
  ...
  it('getOptions', () => {
    ...
    assert.isBoolean(result.emitFile, 'emitFile set');
  });
  ...
  it('collectModules', () => {
    ...
    const expected = { 'module:1': { hello: 'Sei gegrüßt!' } };

    assert.deepEqual(collectModules(['module:1'], map, 'de-DE'), expected, 'collected module values properly');
    assert.deepEqual(collectModules(['module:1'], map, 'es-ES'), {}, 'language missing');
  });

  it('validateKeys', () => {
    const valid = ['de-DE', 'es-ES', 'fr-FR'];
    const invalid = valid.concat('BOO-POOO');

    assert.doesNotThrow(() => validateKeys(valid));
    assert.throws(() => validateKeys(invalid));
  });

  it('createModule', () => {
    const source = '{"de-DE":{"title":"BOAH!","description": "echt"}}';
    const expected = {
      'de-DE': {
        description: 'echt',
        title: 'BOAH!',
    ...
    assert.deepEqual(createModule(source), expected, 'creates module correctly');
  });
});
