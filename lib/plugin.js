...
const PLUGIN_NAME = 'I18nModulesPlugin';
...
class I18nModulesPlugin {
  constructor(_options) {
    const options = getOptions(_options);
    validate(schema, options, { name: 'I18nModules Plugin' });

    const { keysRoot, moduleEnding, emitFile, getId, dictionaryAdapter } = options;
    ...
    this.replacementsOptions = { dictionaryAdapter, emitFile, store: this.replacements };
      ...
      compilation.hooks.finishModules.tapPromise(PLUGIN_NAME, (moduleSet) => {
        ...
        const builtModules = modules.reduce((acc, { resource }) => {
          if (resource?.endsWith(moduleEnding)) {
            ...
            try {
              // Trying to create a JSON object to be stored in the Map
              // Might error due to syntax errors or validation errors
      ...
      if (emitFile) {
        compilation.hooks.processAssets.tap({
          ...
          Array.from(compilation.modules).forEach(({ resource }) => {
            ...
            const source = dictionaryAdapter.stringify(data);
...
I18nModulesPlugin.loader = MODULE_LOADER;
module.exports = I18nModulesPlugin;
